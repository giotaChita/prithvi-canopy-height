...
class CanopyHeightHead(nn.Module):
    def __init__(self, embed_dim, patch_size):
        ...
        def upscaling_block(in_channels, out_channels):
            ...
            if embed_dim == 128:
                return nn.Sequential(
                    ...
                    nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                    nn.Conv2d(kernel_size=3, in_channels=out_channels, out_channels=16, padding=1),
                    nn.ReLU(),
            ...
            if embed_dim == 512:# or embed_dim == 768:
                return nn.Sequential(
                    nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                    nn.Conv2d(kernel_size=3, in_channels=in_channels, out_channels=out_channels, padding=1),
                    nn.ReLU(),
            ...
            #
            #         nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            #         nn.Conv2d(kernel_size=3, in_channels=out_channels, out_channels=128, padding=1),
            #         nn.ReLU(),
            #
            #         nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            #         nn.Conv2d(kernel_size=3, in_channels=128, out_channels=16, padding=1),
            #         nn.ReLU(),
            #
            ...
            # Params: , when 768 -> 128
                    ...
                    return nn.Sequential(
                        ...
                        nn.Upsample(scale_factor=2.1, mode='bilinear', align_corners=False), # 2.1 for size tile 128, 2 FOR TILE SIZE 50
                    )
        ...
        if embed_dim == 1024:
            embed_dims = [embed_dim // (4**i) for i in range(3)]
            self.upscaling_blocks = nn.Sequential(
                *[upscaling_block(embed_dims[i], embed_dims[i+1]) for i in range(2)],
                nn.Upsample(scale_factor=2.5, mode='bilinear', align_corners=False)
            )
            in_channels = embed_dims[-1]

        if embed_dim == 768:
            # Params: 2.083.233-> 256
            # Params: 967.905 -> 128
            self.upscaling_blocks = nn.Sequential(
        ...
        # if embed_dim == 768:
        #     # Params: 2.654.977
        #     embed_dims = [embed_dim // (2**i) for i in range(2)]
        #     self.upscaling_blocks = nn.Sequential(
        #         *[upscaling_block(embed_dims[i], embed_dims[i+1]) for i in range(1)],
        ...
        # if embed_dim == 768:
        #     # Params:3.318.529
        #     embed_dims = [embed_dim // (2**i) for i in range(3)]
        #     self.upscaling_blocks = nn.Sequential(
        #         *[upscaling_block(embed_dims[i], embed_dims[i+1]) for i in range(2)],
        #         nn.Upsample(scale_factor=2.5, mode='bilinear', align_corners=False)
        #     )
